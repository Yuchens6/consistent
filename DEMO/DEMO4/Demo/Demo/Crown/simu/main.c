...
void load()
{
   ...
   HINSTANCE hDLL = LoadLibraryA("sgpi-opengl.dll");
    ...
    sgpi_draw_text = (SGPI_TEXT)GetProcAddress(hDLL, "sgpi_draw_text");*/
    sgpi_scissor = (SGPI_SCISSOR)GetProcAddress(hDLL, "sgpi_scissor");
    sgpi_clip_plane = (SGPI_CLIP_PLANE)GetProcAddress(hDLL, "sgpi_clip_plane");
    sgpi_clip_box = (SGPI_CLIP_BOX)GetProcAddress(hDLL, "sgpi_clip_box");
    sgpi_clip_stencil = (SGPI_CLIP_STENCIL)GetProcAddress(hDLL, "sgpi_clip_stencil");
...
{
    pointer_event.pressed = (GLboolean) pressed;
    pointer_event.released = (GLboolean) released;
...
{
    keyboard_event.pressed = (GLboolean) pressed;
    keyboard_event.released = (GLboolean) released;
