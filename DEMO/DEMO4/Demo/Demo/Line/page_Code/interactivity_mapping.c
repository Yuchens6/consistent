...
void aol_pointer(int pPointer, GLboolean* pPressed, GLboolean* pReleased, int* pButton, float* pX, float* pY, int* pModifiers) {
  int index;
  for (index=0; index<last_pointer_event; index++) {
    const sdy_pointer_event_t *lPointerEvent = &pointer_events[index];
    if ((pPointer == -1) || (lPointerEvent->id == -1) || (pPointer == lPointerEvent->id)) {
...
void aol_keyboard(int pKeyboard, GLboolean* pPressed, GLboolean* pReleased, long* pCode, int* pModifiers) {
  int index;
...
Point cal_translation(Point start, Point end, Point reference, Point origin, double startValue, double endValue, GLboolean startLocked, GLboolean endLocked, double funValue){
	...
	if(startLocked)
	{
		if(funValue < startValue)
			funValue = start.x;
	}
    if(endLocked)
	{
		if(funValue > endValue)
			funValue = endValue;
	}
	currPoint.x = origin.x - reference.x + start.x + (funValue - startValue) / (endValue - startValue) * (end.x - start.x);
    currPoint.y = origin.y - reference.y + start.y + (funValue - startValue) / (endValue - startValue) * (end.y - start.y);
    return currPoint;
}
